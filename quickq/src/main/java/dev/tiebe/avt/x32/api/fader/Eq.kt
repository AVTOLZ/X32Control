...
open class Eq(val fader: Fader) {
    ...
    fun setState(state: Boolean) {
        fader.quickQOSC.sendMessage(OSCMessage(stateOSCMessage, listOf(if (state) 1 else 0)))
    }
    ...
    suspend fun getType(band: Int): BiQuadraticFilter.Companion.FilterType {
        ...
        val type = fader.quickQOSC.getValue(OSCMessage(typeOSCMessage.format(band)))?.arguments?.get(0) as Int
    ...
    fun setType(band: Int, type: BiQuadraticFilter.Companion.FilterType) {
        ...
        fader.quickQOSC.sendMessage(OSCMessage(typeOSCMessage.format(band), listOf(getFilterValue(type))))
    }
    ...
    suspend fun getFrequency(band: Int): Float {
        ...
        return (fader.quickQOSC.getValue(OSCMessage(frequencyOSCMessage.format(band)))?.arguments?.get(0) as Float)
    }
    ...
    fun setFrequency(band: Int, frequency: Float) {
        ...
        fader.quickQOSC.sendMessage(OSCMessage(frequencyOSCMessage.format(band), listOf(frequency)))
    }
    ...
    suspend fun getGain(band: Int): Float {
        ...
        return (fader.quickQOSC.getValue(OSCMessage(gainOSCMessage.format(band)))?.arguments?.get(0) as Float)
    }
    ...
    fun setGain(band: Int, gain: Float) {
        ...
        fader.quickQOSC.sendMessage(OSCMessage(gainOSCMessage.format(band), listOf(gain)))
    }
    ...
    suspend fun getQ(band: Int): Float {
        ...
        return (fader.quickQOSC.getValue(OSCMessage(qOSCMessage.format(band)))?.arguments?.get(0) as Float)
    }
    ...
    fun setQ(band: Int, q: Float) {
        ...
        fader.quickQOSC.sendMessage(OSCMessage(qOSCMessage.format(band), listOf(q)))
    }
