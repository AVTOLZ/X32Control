package dev.tiebe.avt.osc.quickq.api.internal
...
class Status private constructor(private val osc: dev.tiebe.avt.osc.quickq.QuickQOSC) {
    ...
    suspend fun getSelection(): Fader {
        ...
        return osc.getFaderFromIndex((value.arguments[0] as Int + 1).toString()) ?: throw IllegalStateException("Could not get selection")
    }
    ...
    companion object {
        @Volatile private var INSTANCES: MutableMap<dev.tiebe.avt.osc.quickq.QuickQOSC, Status> = mutableMapOf()

        fun getInstance(osc: dev.tiebe.avt.osc.quickq.QuickQOSC): Status {
